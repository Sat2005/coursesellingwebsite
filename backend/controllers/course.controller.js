...
export const createCourse = async (req, res) => {
  ...
  try {
    ...
    if (!cloud_response || cloud_response.error) {
      return res.status(400).json({ errors: `Error uploading file to Cloudinary: ${cloud_response.error?.message || 'Unknown error'}` });
    }
    ...
    res.status(500).json({ error: `Error creating course: ${error.message || 'Unknown error'}` });
  }
};
